...
// Create a form element from form specification
function createFormBubble(formSpec) {
  const messageDiv = document.createElement("div");
  messageDiv.className = "message agent form-message";

  const bubbleDiv = document.createElement("div");
  bubbleDiv.className = "bubble form-bubble";
  ...
  if (formSpec.description) {
    ...
    desc.textContent = formSpec.description;
  ...
  form.setAttribute("data-form-id", formSpec.form_id);
  form.setAttribute("data-submit-tool", formSpec.submit_tool || "");

  // Create form fields
  if (formSpec.fields && Array.isArray(formSpec.fields)) {
  ...
  submitBtn.textContent = formSpec.submit_button || "Submit";
  form.appendChild(submitBtn);

  // Add form submit handler
  form.addEventListener("submit", function (e) {
    e.preventDefault();
    handleFormSubmit(form, formSpec);
  });

  formContainer.appendChild(form);
  bubbleDiv.appendChild(formContainer);
  messageDiv.appendChild(bubbleDiv);
...
function handleFormSubmit(form, formSpec) {
  ...
  // Send form data to the agent via WebSocket
  if (websocket && websocket.readyState === WebSocket.OPEN) {
    const toolName = formSpec.submit_tool || "create_task_from_form";
    const message = JSON.stringify({
      type: "text",
      text: `[FORM_SUBMISSION] ${toolName}: ${JSON.stringify(data)}`
    });
    ...
    confirmMsg.className = "system-message";
    confirmMsg.textContent = `Form submitted: ${formSpec.title}`;
    messagesDiv.appendChild(confirmMsg);
    scrollToBottom();
  }
}

// Try to parse JSON from text, returns null if not valid JSON
  ...
  websocket.onmessage = function (event) {
    ...
    if (adkEvent.content && adkEvent.content.parts) {
      ...
      for (const part of parts) {
        ...
        // Handle function responses — AgentTool wraps the sub-agent's text
        // result as { result: "..." } inside functionResponse, not as a text
        // part, so we must check here for form JSON.
        if (part.functionResponse && part.functionResponse.response) {
          const result = part.functionResponse.response.result;
          if (result && typeof result === 'string') {
            const formSpec = tryParseJSON(result);
            if (formSpec && formSpec.type === 'form' && formSpec.form_id) {
              console.log('✓ Form detected in functionResponse:', formSpec);
              const formMessageId = Math.random().toString(36).substring(7);
              const formBubble = createFormBubble(formSpec);
              formBubble.id = formMessageId;
        ...
        if (part.text) {
          ...
          // Check for form spec before any other checks — forms must always be rendered
          // even in audio mode (hasOutputTranscriptionInTurn). Form JSON arrives as
          // streaming partial chunks: partial chunks fail to parse and create a text
          // bubble; the final complete event carries valid JSON and we must replace the
          // partial text bubble with the actual form.
          const formSpec = tryParseJSON(part.text);
          if (formSpec && formSpec.type === "form" && formSpec.form_id) {
            console.log("✓ Form detected:", formSpec);
            if (currentMessageId == null) {
              currentMessageId = Math.random().toString(36).substring(7);
              currentBubbleElement = createFormBubble(formSpec);
              currentBubbleElement.id = currentMessageId;
              messagesDiv.appendChild(currentBubbleElement);
            } else {
              // Partial chunks created a garbled text bubble — replace it with the form
              const existingBubble = document.getElementById(currentMessageId);
