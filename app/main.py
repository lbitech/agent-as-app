...
async def websocket_endpoint(
    ...
    async def downstream_task() -> None:
        ...
        async for event in runner.run_live(
            ...
            # Detailed logging for function calls and responses
            if event.content and event.content.parts:
                for p in event.content.parts:
                    if p.function_call:
                        print(f"[FUNC_CALL] author={event.author} name={p.function_call.name} args={p.function_call.args}")
                    if p.function_response:
                        print(f"[FUNC_RESP] author={event.author} name={p.function_response.name} response={str(p.function_response.response)[:300]}")
                    if p.text and not p.thought:
                        print(f"[TEXT] author={event.author} partial={event.partial} text={p.text[:200]}")
